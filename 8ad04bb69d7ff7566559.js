...
const createSnake = (chain, { sizeCell, sizeDot }, duration) => {
    ...
    const svgElements = snakeParts.map((_, i, { length }) => {
        ...
        // 'font-size': '1rem',
        (0,_utils__WEBPACK_IMPORTED_MODULE_1__.t)('text', {
            class: `s s${i}`,
            x: m.toFixed(1),
            y: m.toFixed(1),
            width: s.toFixed(1),
    ...
    const transform = ({ x, y }) => `transform:translate(${x * sizeCell}px,${y * sizeCell}px) rotate(${x}deg)`;
    const styles = [
        `.s{ 
      shape-rendering:geometricPrecision;
      fill:var(--cs);
...
const t = (element, attributes, content) => `<${element} ${toAttribute(attributes)}>${content}</${element}>`;
